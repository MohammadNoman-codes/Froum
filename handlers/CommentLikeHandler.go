...
func CommentLikeHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err = tx.Exec("DELETE FROM comment_dislikes WHERE user_id = ? AND comment_id = ?", userID, commentID)
	if err != nil {
	...
	// Insert like
	_, err = tx.Exec("INSERT INTO comment_likes (user_id, comment_id) VALUES (?, ?)", userID, commentID)
	if err != nil {
